...
- Não é para o `tar` manter a estrutura original dos arquivos (diretórios), é para todos os arquivos informados como argumento, ficar no diretório raiz do `tar`.
- As partituras deve ser renomeadas dentro do `tar`. As originais não podem ser renomeadas.
